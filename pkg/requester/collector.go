...
type Collector struct {
	...
	// Statistics
	totalRequests atomic.Int64  // Total requests actually sent
	successful    atomic.Int64
	failed        atomic.Int64

	// Poisson arrival tracking (only used in Poisson mode)
	generatedRequests atomic.Int64  // Total arrivals generated by Poisson process
	droppedRequests   atomic.Int64  // Requests dropped due to full queue
			...
			for {
				if usePoisson {
					...
					case <-timer.C:
						...
						// Count this as a generated arrival (Poisson process)
						c.generatedRequests.Add(1)

						// Try to send to queue (non-blocking)
						select {
						case queue <- struct{}{}:
							// Queued successfully - will be sent later
						case <-ctx.Done():
							return
						default:
							// Queue is full - drop this request and count it
...
func (c *Collector) calculateStats(duration float64, totalReqs, failed int64, actualQPS float64) RequestStats {
	...
	// Add Poisson arrival metrics if in Poisson mode
	generated := c.generatedRequests.Load()
	dropped := c.droppedRequests.Load()
	if generated > 0 {
		stats.GeneratedRequests = generated
		stats.DroppedRequests = dropped
		stats.DropRate = float64(dropped) / float64(generated) * 100.0
		if duration > 0 {
			stats.TargetArrivalRate = float64(generated) / duration
		}
	}
	...
	if successfulReqs > 0 && duration > 0 {
		...
		// For Poisson mode, use target arrival rate; otherwise use actual QPS
		arrivalRate := actualQPS
		if generated > 0 && stats.TargetArrivalRate > 0 {
			arrivalRate = stats.TargetArrivalRate
		}
		...
		if avgResponseTimeSec > 0 {
			...
			stats.Utilization = arrivalRate / serviceRate
		}
	}
